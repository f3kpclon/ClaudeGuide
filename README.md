...
**Author:** Félix Sotelo · **Version:** v5.31 · Re-verified against official docs (2026-09-02) — **Opus 5** (`claude-opus-5`) replaces Opus 4.8 as the current Opus and is the account default on Max/Team Premium/Enterprise/API; **Fable 5.1** tops the lineup. Sonnet 5's introductory $2/$10 **became the standard price** — the September 1, 2026 increase to $3/$15 was cancelled, so the Opus:Sonnet ratio is 2.5× and no longer expires (6 `vence:` markers cleared). §25 fixes a factual error: **fast mode IS an API parameter** (`speed: "fast"` + beta `fast-mode-2026-02-01`, Opus 5/4.8; Opus 4.6 silently ignores it). New: `xhigh` doesn't exist on Opus 4.6 or Sonnet 4.6; Claude Code aliases (`best`/`fable`/`opusplan`/`[1m]`) and the 4 ways to set effort; §3 adds the **4.7+ tokenizer (~30% more tokens)** and the 1h cache write; Haiku 4.5 retires no sooner than 2026-10-15
