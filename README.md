...
| Area | Change |
|---|---|
| **§7** | **New — the shunt hook: `PreToolUse` used for economics, not safety.** A guard on `Read` that does not forbid but **redirects**: a full read of a large file is denied, and `permissionDecisionReason` tells Claude to delegate to a cheap subagent or re-read with `offset`/`limit`. The allow-list is the design: targeted reads, small files and missing files all pass — only the blind full read pays. Includes the rule most people miss: **a guard on `Read` without its twin on `Bash` lasts one turn**, because `cat`/`head`/`tail` route straight around it. |
| **§23** | **A fourth way out of the ceiling.** The section treated `Σ(tool_outputs)` as a constant of the operation. It is a constant of the operation **times where it runs**: a subagent reads in isolation and returns only its report, so the corpus never enters the parent context — this turn or any later one. New lever #6, with the threshold table for when it does *not* apply (editing, debugging, small files, judgment calls). |
| **§19** | **New — testing the proxy instead of the contract.** A public plugin ships 51 green tests over a gate whose contract is never exercised: each test asks the hook script what the hook script printed, and the three tests that would cross the real harness are versioned but never run by the runner. Rule that follows: every gate needs **at least one test that goes through the real harness**, and **one negative test** — the case that must pass through. |
| **§11** | **Correction — a non-whitelisted directory does travel.** The guide claimed a `scripts/` dir "may not survive installation". It does: installation copies the plugin directory verbatim, verified against a third-party plugin already installed from a marketplace (`scripts/` with 9 executables plus `tests/`, neither whitelisted). The whitelist governs **what is registered as a component**, not **what is copied**. The recommendation stands as ergonomics; the justification was counterfactual. |
