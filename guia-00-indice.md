...
**Versión:** v5.36 · §26 re-verificado (2026-09-02) — **el bloque publicado llevaba dos meses desincronizado del hook instalado**: seguía con `GUIA = Path(".../guia-agentes-plugins-claude-code.md")`, el archivo único que dejó de existir en el split v5.16, así que quien copiaba la receta armaba un hook apuntando a un archivo inexistente. El audit solo comparaba KEYWORD_MAP y CAP_CHARS — **check 10 nuevo** que compara el bloque entero. Física de la inyección verificada: solo `UserPromptSubmit`/`UserPromptExpansion`/`SessionStart`/`PostModelSwitch` convierten stdout plano en contexto; **timeout de 30 s** (no 10 min); `UserPromptSubmit` **no soporta matcher**; `additionalContext` fuera de `hookSpecificOutput` **se ignora en silencio**; y un `echo` del `.zshrc` antepuesto al stdout puede romper un hook que devuelva JSON sin reportar nada en exit 0. `SessionStart` tiene un quinto source, `fork` (§7 y §33 corregidos) — 2026-09-02
